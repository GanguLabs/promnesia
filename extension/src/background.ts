...
import type { SearchPageParams, Url } from "./common";
import {
    assert,
    Blacklisted,
    Methods,
...
import { getOptions, setOption, THIS_BROWSER_TAG, Toggles } from "./options";

import { Filterlist } from "./filterlist";
import { defensify, notifications, Notify, notifyError } from "./notifications";
import { allsources, isAndroid as isMobile } from "./sources";
...
function getIconStyle(result: Result): IconStyle {
    if (result instanceof Blacklisted) {
        return {
            icon: "images/ic_blacklisted_48.png",
            title: `Blacklisted: ${result.reason}`,
            text: "",
        };
    }
    ...
    if (errs.length > 0) {
        return {
            icon: "images/ic_error.png",
            title: `${errs.length} errors`,
            text: errs.map((x) => x.toString()).join("\n"),
        };
    }
    ...
    if (vcount === 0) {
        return {
            icon: "images/ic_not_visited_48.png",
            title: "No data",
            text: "",
        };
    }
    ...
    if (ccount > 0) {
        return {
            icon: "images/ic_visited_48.png",
            title: `${vcount} visits, ${ctext}`,
            text: btext,
        };
    }
    if (rcount > 0) {
        ...
        return {
            icon: "images/ic_relatives_48.png",
            title: `${vcount} visits, ${ctext}`,
            text: btext,
        };
    }
    // TODO a bit ugly, but ok for now.. maybe cut off by time?
    const boring = good.every(
        (v) => v.tags.length == 1 && v.tags[0] == THIS_BROWSER_TAG
    );
    if (boring) {
        // TODO not sure if really worth distinguishing..
        return {
            icon: "images/ic_boring_48.png",
            title: `${vcount} visits (${THIS_BROWSER_TAG} only)`,
            text: "",
        };
    } else {
        return {
            icon: "images/ic_blue_48.png",
            title: `${vcount} visits`,
            text: "",
        };
    }
...
async function updateState(options: {
    ...
    if (!options.navigationError) {
        ...
        if (!sidebar_injected) {
            console.debug(
                "cancelling state update request for %o -- likely URL changed during processing",
                tab
            );
            return;
        }
    ...
    const isOk =
        (await browser.tabs.get(tabId)).favIconUrl !=
    ...
    if (isOk) {
        ...
        console.warn(
            "TODO implement binding visits to popup? or at least show error message"
        );
    }
}
...
async function sendSidebarMessage(tabId: number, message: any) {
    ...
    */
    ...
    for (let i = 0; i < 100; i++) {
        // wait for 100 attemps x 100 ms = 10 seconds total, kind of arbitrary
        try {
...
async function doToggleMarkVisited(
    tabId: number,
    { show }: { show: boolean | null } = { show: null }
) {
    ...
    for (const url of results) {
        // NOTE: traverse over original results, duplicates need to be taken into the account
        if (url == null) {
            continue;
        }
...
async function handleWebNavigationEvent(detail: WebNavigationDetails) {
    ...
    try {
        await updateState({
            tab: { url: url, id: detail.tabId },
            navigationError: "error" in detail,
        });
    } catch (error) {
        ...
        if (message.includes("Invalid tab ID")) {
            console.warn(
                "Error %o ignored; most likely due to closed tab",
                error
            );
            return;
        }
        if (message.includes("An unexpected error occurred")) {
            console.warn(
                "Error %o ignored; presumably bug in Firefox https://bugzilla.mozilla.org/show_bug.cgi?id=1397667",
                error
            );
...
browser.webNavigation.onErrorOccurred.addListener(
    defensify(async (detail: WebNavigation.OnErrorOccurredDetailsType) => {
...
browser.webNavigation.onCompleted.addListener(
    defensify(async (detail: WebNavigation.OnCompletedDetailsType) => {
        await handleWebNavigationEvent(detail);
...
async function shouldProcessPage(
    tab: TabUrl | null
): Promise<ShouldProcess | null> {
    if (tab == null) {
        await notifications.page_ignored(
            null,
            null,
            "Couldn't determine current tab: must be a special page (or a bug?)"
        );
        return null;
    }
    const url = tab.url!;
...
const onMessageCallback = (message: any): Promise<any> | void => {
    // TODO not sure if should defensify here?
...
const onMessageCallbackSeleniumBridge = (
    message: any,
...
function registerActions() {
    ...
    for (const action of actions()) {
        action.onClicked.addListener(
            defensify(
                (tab: Tabs.Tab) =>
                    toggleSidebarOnTab({ url: tab.url!, id: tab.id! }),
                "action.onClicked"
            )
        );
    }
...
async function handleAddToGlobalExcludelist() {
    ...
    */
    ...
    await Notify.notify(
        tabId,
        `Added ${String(added)} to blacklist (${ll} items now)`,
        { color: "blue" }
    );
...
const AddToMarkVisitedExcludelist = {
        ...
        await executeScript({
            ...
            func: (method_zapper_excludelist: string) => {
                ...
                document.addEventListener("click", (e: MouseEvent) => {
                    ...
                    // @ts-expect-error
                    let links = Array.from(e.target.getElementsByTagName("a"))
                    ...
                    chrome.runtime.sendMessage({
                        method: method_zapper_excludelist,
                        data: links,
                    });
                });
    ...
    add: async function (urls: Array<Url>) {
        ...
        if (urls.length == 0) {
            await Notify.error(tabId, "No URLs were detected");
            return;
        }
...
type MenuEntry = {
    ...
    title: string;
    callback:
        | ((info: Menus.OnClickData, tab: Tabs.Tab) => Promise<void>)
        | null;
...
function hasContextMenus(): boolean {
    ...
    if (browser.contextMenus == undefined) {
        isMobile().then((mobile) => {
            if (!mobile) {
                notifyError(
                    new Error("chrome.contextMenus should be available")
                );
            }
        });
        return false;
...
function initContextMenus(): void {
    ...
    */
    ...
    const onMenuClickedCallback = defensify(
        async (info: Menus.OnClickData, tab: Tabs.Tab | undefined) => {
            assert(tab != null);
...
async function updateContextMenus(opts: Options): Promise<void> {
    ...
    for (const { id: id, checker: checker } of TOGGLES) {
        browser.contextMenus.update(id, { checked: checker(opts) });
    }
}
